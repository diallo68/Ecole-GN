  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-ink-muted">{users.length} compte{users.length > 1 ? 's' : ''} affiché{users.length > 1 ? 's' : ''}</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-ink/10 overflow-hidden">
        {loading ? (
          ...
          <p className="p-4 text-sm text-ink-muted">Aucun inscrit ne correspond.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-ink-muted text-xs border-b border-ink/10">
                  ...
                  <tr key={u._id} className="border-b border-ink/5 last:border-0 hover:bg-sand/60">
                    <td className="px-4 py-3">
                      {u.photo ? (
                        ...
                        <div className="w-8 h-8 rounded-full bg-sand text-ink-muted text-xs font-bold grid place-items-center border border-ink/10">
                          {u.prenom?.[0]}{u.nom?.[0]}
                        </div>
                      )}
                    </td>
                    <td className="px-4 py-3 font-semibold text-ink whitespace-nowrap">{u.prenom} {u.nom}</td>
                    <td className="px-4 py-3 text-ink-muted whitespace-nowrap">{u.email}</td>
                    <td className="px-4 py-3 text-ink-muted whitespace-nowrap">{u.phone || '—'}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-ink-muted whitespace-nowrap">{u.city || '—'}</td>
                    ...
                    <td className="px-4 py-3 text-ink-muted whitespace-nowrap">{new Date(u.createdAt).toLocaleDateString('fr-FR')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
