...
export default function AdminRepetiteursPage() {
  ...
  const moderer = async (id: string, valide: boolean) => {
    try {
      ...
      toast.success(valide ? 'Répétiteur validé !' : 'Répétiteur refusé');
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">Modération des répétiteurs</h1>

      <div className="flex gap-2 mb-6">
        {(['en_attente', 'valide', 'tous'] as Filtre[]).map(f => (
          <button key={f} onClick={() => setFiltre(f)}
            className={`px-3 py-1.5 rounded-lg text-sm font-semibold capitalize ${filtre === f ? 'bg-brand text-white' : 'bg-white border border-slate-300'}`}>
            {f.replace('_', ' ')}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <p className="text-slate-500">Aucun répétiteur dans cette catégorie.</p>
      ) : (
        <div className="space-y-3">
          {repetiteurs.map(r => (
            <div key={r._id} className="bg-white rounded-xl border border-slate-200 p-4 flex items-center justify-between">
              <div>
                <p className="font-semibold">{r.prenom} {r.nom} <span className="text-slate-400 font-normal text-sm">— {r.email}</span></p>
                <p className="text-sm text-slate-500">{r.city} · {r.repetiteur.matieres?.join(', ')} · {r.repetiteur.niveaux?.join(', ')}</p>
              ...
              <div className="flex gap-2 shrink-0">
                ...
                {r.repetiteur.valide && (
                  <button onClick={() => moderer(r._id, false)} className="bg-slate-200 text-slate-700 text-sm font-semibold px-3 py-1.5 rounded-lg hover:bg-slate-300">Suspendre</button>
                )}
              </div>
            </div>
          ))}
