  ...
  if (value) {
    ...
    return (
      <div className="flex items-center gap-2 border border-ink/15 rounded-lg px-3 py-2 text-sm bg-white">
        ...
        <button type="button" onClick={() => { onChange(''); setFileName(''); }} className="text-ink/40 hover:text-flag shrink-0">
          <X size={15} />
        </button>
      </div>
    );
  ...
  return (
    <div>
      ...
      <button type="button" onClick={() => inputRef.current?.click()} disabled={uploading}
        className="w-full flex items-center justify-center gap-2 border border-dashed border-ink/20 rounded-lg px-3 py-2 text-sm font-medium text-ink/60 hover:border-brand hover:text-brand transition-colors disabled:opacity-60">
